...
function initCommunity(app,pool,auth){
  ...
  app.post("/api/tournaments/:id/join",auth,async(req,res)=>{
    ...
    try{
      ...
      await client.query("INSERT INTO tournament_entries(tournament_id,user_id,entry_fee_usdt) VALUES($1,$2,$3)",[t.id,req.user.id,fee]);if(fee>0)await client.query("INSERT INTO wallet_transactions(user_id,type,usdt_amount) VALUES($1,$2,$3)",[req.user.id,"tournament_entry",-fee]);
