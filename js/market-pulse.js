...
function pulseItems(list,positive){
...
function renderPulse(coins){
 ...
 if(pulseGainers)pulseGainers.innerHTML=pulseItems(gainers,true)||'<div class="market-pulse-empty">No data</div>';
 if(pulseLosers)pulseLosers.innerHTML=pulseItems(losers,false)||'<div class="market-pulse-empty">No data</div>';
...
window.addEventListener("gugee:liveCoins",e=>renderPulse(e.detail||[]));
setTimeout(()=>{
 if(Array.isArray(window.liveCoins))renderPulse(window.liveCoins);
},1000);
})();
