...
function pulseMoney(v){const n=Number(v);if(!Number.isFinite(n))return"$0.00";if(n>=1000)return"$"+n.toLocaleString("en-US",{maximumFractionDigits:2});if(n>=1)return"$"+n.toLocaleString("en-US",{maximumFractionDigits:4});return"$"+n.toLocaleString("en-US",{maximumFractionDigits:8});}
function pulseCompact(v){const n=Number(v);if(!Number.isFinite(n))return"$0.00";return"$"+new Intl.NumberFormat("en-US",{notation:"compact",maximumFractionDigits:2}).format(n)}
...
(function(){
const API_BASES=[...new Set([(window.GUGEE_API_BASE||"").replace(/\/$/,""),window.location.origin.replace(/\/$/,""),"https://gugee.onrender.com"])].filter(Boolean);
function baseFetch(path){return window.gugeeMarketData.fetch(path).catch(()=>null)}
function money(v){const n=Number(v);if(!Number.isFinite(n))return"$0.00";if(n>=1000)return"$"+n.toLocaleString("en-US",{maximumFractionDigits:2});if(n>=1)return"$"+n.toLocaleString("en-US",{maximumFractionDigits:4});return"$"+n.toLocaleString("en-US",{maximumFractionDigits:8})}
function compact(v){const n=Number(v);return Number.isFinite(n)?"$"+new Intl.NumberFormat("en-US",{notation:"compact",maximumFractionDigits:2}).format(n):"0.00%"}
...
function mainCard(prefix,c){
 ...
 el.textContent=Number.isFinite(ch)?(ch>=0?"+":"")+ch.toFixed(2)+"%":"0.00%";el.className=ch>=0?"positive":"negative";
...
async function get(path){return window.gugeeMarketData.fetch(path).catch(()=>null)}
function money(v){const n=Number(v);return Number.isFinite(n)?"$"+n.toLocaleString("en-US",{maximumFractionDigits:2}):"0.00%"}
function compact(v){const n=Number(v);return Number.isFinite(n)?"$"+new Intl.NumberFormat("en-US",{notation:"compact",maximumFractionDigits:2}).format(n):"0.00%"}
async function sentiment(){
 const d=await get("/api/market/fear-greed");
 const x=d?.data?.[0]; if(!x)return;
 const value=Number(x.value); document.getElementById("sentimentValue").textContent=Number.isFinite(value)?value:"0.00%";
 document.getElementById("sentimentLabel").textContent=x.value_classification||"Unknown";
 document.getElementById("sentimentTime").textContent=x.timestamp?new Date(Number(x.timestamp)*1000).toLocaleString():"0.00%";
...
async function globalStats(){
 ...
 const capEl=document.getElementById("pulseMarketCapChange");capEl.textContent=Number.isFinite(capChange)?(capChange>=0?"+":"")+capChange.toFixed(2)+"%":"0.00%";capEl.className=capChange>=0?"positive":"negative";
 const dom=Number(x.market_cap_percentage?.btc);document.getElementById("pulseBtcDominance").textContent=Number.isFinite(dom)?dom.toFixed(2)+"%":"0.00%";document.getElementById("dominanceFill").style.width=Math.max(0,Math.min(100,dom||0))+"%";
}
