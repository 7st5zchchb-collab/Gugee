...
function render(){
 ...
 if(!list.length){body.innerHTML='<tr><td colspan="6" class="market-loading">No cryptocurrencies found.</td></tr>';return}
 body.innerHTML=list.map(c=>{const ch=c.price_change_percentage_24h;return '<tr class="market-row" data-id="'+c.id+'"><td class="rank">'+c.market_cap_rank+'</td><td><div class="asset"><img src="'+c.image+'" alt=""><div><b>'+c.name+'</b><span>'+c.symbol.toUpperCase()+'</span></div></div></td><td><b>'+money(c.current_price)+'</b></td><td class="'+(ch>=0?"positive":"negative")+'">'+(ch>=0?"+":"")+(ch==null?"--":ch.toFixed(2)+"%")+'</td><td>'+money(c.market_cap)+'</td><td>'+compact(c.total_volume)+'</td></tr>'}).join("");
 document.querySelectorAll(".market-row").forEach(r=>r.onclick=()=>location.href="crypto.html?coin="+encodeURIComponent(r.dataset.id));
 more.style.display=list.length<coins.length&&list.length<visible+1?"":"none";
}
async function load(){try{const r=await fetch(API);if(!r.ok)throw new Error("API failed");coins=await r.json();status.textContent="Live CoinGecko data";render()}catch(e){console.error(e);body.innerHTML='<tr><td colspan="6" class="market-loading">Market data could not be loaded. Try again.</td></tr>';status.textContent="Data unavailable"}}
